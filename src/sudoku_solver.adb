...
procedure Sudoku_Solver is
   ...
   function Is_Valid_Square_Input
     (Input : String; Input_Last : Natural) return Boolean is
   begin
      if Input_Last /= 9 then
         return False;
      end if;

      for I in 1 .. 9 loop
         if not (Input (I) in '1' .. '9' or else Input (I) = '_') then
            return False;
         end if;
      end loop;

      return True;
   end Is_Valid_Square_Input;
begin
   ...
   loop
      ...
      exit when Last > 0 and then Entries (1) = 'q';

      if not Is_Valid_Square_Input (Entries, Last) then
         Put_Line ("Enter exactly 9 characters using digits 1-9 or _.");
         Squares := Squares - 1;
         New_Line;
      else
         declare
            -- Parse the 9 numbers for the current 3x3 block
            Temp_Square : Sudoku_Engine.Specific_Square :=
              Sudoku_Engine.Parse_Square (Entries, Last);
         ...
         begin
            for I in 1 .. 9 loop
               declare
                  ...
                  Val : Integer := 0;
               begin
                  if Temp_Square (I).Count = 1 then
                     Val := Temp_Square (I).Data (1);
                  end if;

                  My_Board.Set_Global_Square
                    (Row => Global_R, Col => Global_C, Value => Val);
               end;
            end loop;
